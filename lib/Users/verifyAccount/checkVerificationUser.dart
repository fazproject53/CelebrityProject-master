...
class checkVerificationUser extends StatefulWidget {

  File? file;
  String? accountType;
  checkVerificationUser({Key? key, this.accountType, this.file,}) : super(key: key);
  _checkVerificationUserState createState() => _checkVerificationUserState();
}

class _checkVerificationUserState extends State<checkVerificationUser> {
  bool isValue1 = false;
  int? _value;
  bool edit = false;
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting ) {
                  ...
                  if (snapshot.hasError) {
                    ...
                    snapshot.data!.data!.user!.verifiedFile == null? null:{

                      name =snapshot.data!.data!.user!.verifiedFile!.split('/').last,

                      file =  downloadFile(snapshot.data!.data!.user!.verifiedFile!, name!)};
                    widget.accountType == null? widget.accountType = snapshot.data!.data!.user!.celebrityType!: null;
                            ...
                            children: [
                              ...
                              Stack(
                                ...
                                children: [
                                  ...
                                  snapshot.data!.data!.user!.verifiedRejectReson == ''?Image.asset('assets/image/check.png', height: 30.h): SizedBox(),
                                ],
                              ),
                              SizedBox(height: 35.h,),
                              text(context,snapshot.data!.data!.user!.verifiedStatus!.name! , textHeadSize, greyBlack, align: TextAlign.center),
                              //+
                              snapshot.data!.data!.user!.verifiedRejectReson ==  ''?
                              text(context,'' , 0, greyBlack, align: TextAlign.center)
                                  :text(context,' بسبب '+ snapshot.data!.data!.user!.verifiedRejectReson! , textHeadSize, greyBlack, align: TextAlign.center),
                              SizedBox(height: edit? 85.h :135.h,),
                              SizedBox(height: file2 == null && snapshot.data!.data!.user!.verifiedRejectReson ==  ''?50.h:0.h),
                              SizedBox(height: file2 != null?100.h : snapshot.data!.data!.user!.verifiedRejectReson ==  ''?130.h:110,),
                              file2 != null?InkWell(
                                onTap: () async {
                                ...
                                child: Container(margin:EdgeInsets.only(right: 25.w),alignment: Alignment.centerRight,
                                  ...
                                  child: Column(
                                    children: [
                                      InkWell(
                                        onTap: () async {
                                          await launch(snapshot.data!.data!.user!.verifiedFile!);
                                        },
                                        child: Row(
                                          children: [
                                            Icon(attach,size: 20.h, color: purple,),
                                            text(context, Path.basename(snapshot.data!.data!.user!.verifiedFile!.length > 21?snapshot.data!.data!.user!.verifiedFile!.substring(80):snapshot.data!.data!.celebrity!.verifiedFile!), textSubHeadSize, greyBlack, align: TextAlign.right),
                                          ],
                                        ),
                                      ),
                                      SizedBox(height: 20.h,)
                              ...
                              snapshot.data!.data!.user!.verifiedRejectReson ==  ''? SizedBox() :
                              InkWell(
                                onTap: (){
                                      ...
                                      showDialog(
                                        ...
                                        builder: (BuildContext context2) {
                                          ...
                                          updateVerify().then((value) => {
                                            value.contains('true')
                                                ? {
                                              gotoPageAndRemovePrevious(context2,  checkVerificationUser(accountType: widget.accountType,file: file2,)),
                                              //done
